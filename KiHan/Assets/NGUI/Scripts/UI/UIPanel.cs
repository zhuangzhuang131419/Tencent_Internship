...
public class UIPanel : UIRect
{
	...
	{
		//UpdateTransformMatrix();  // 注释掉这个保证只有一个，有可能隔帧,  yorkwen
	...
	void LateUpdate ()
	{
		if (mUpdateFrame != Time.frameCount)
		{
			...
			for (int i = 0; i < list.size; ++i)
			{
                ...
                else if (p.renderQueue == RenderQueue.PlaceHolder)
                {
                    rq = Mathf.Max(rq, p.startingRenderQueue + 1);
                }
                else if (p.renderQueue == RenderQueue.PlaceHolderRange)
                {
                    p.startingRenderQueue = rq;
                    rq += Mathf.Max(1, p.placeholderSize);
                }
                else // Explicit
                {
                    p.UpdateDrawCalls();
	...
	bool FillDrawCall (UIDrawCall dc)
	{
		if (dc != null)
		{
			...
			for (int i = 0; i < widgets.size; )
			{
				...
				{
#if UNITY_EDITOR
					Debuger.LogError("This should never happen");
#endif
					widgets.RemoveAt(i);
					continue;
				}
	...
	void UpdateWidgets()
	{
        ...
        Matrix4x4 panelWorldToLocal = this.worldToLocal;
        // Update all widgets
        for (int i = 0, imax = widgets.size; i < imax; ++i)
		{
			...
			if (w.panel == this && w.enabled)
			{
				...
				if (w.UpdateTransform(frame, panelWorldToLocal) || mResized)
				{
	...
	public Vector2 GetViewSize ()
	{
		...
		if (!clip)
		{
            ...
            {
                size *=
                rt.GetPixelSizeAdjustment(Mathf.RoundToInt((rt.scalingStyle == UIRoot.Scaling.FixedSizeofWidth) ? size.x : size.y));
            }
#else
			if (rt != null) size *= rt.GetPixelSizeAdjustment(Screen.height);
#endif
	...
	static protected Vector2 GetMainGameViewSize ()
	{
		int frame = Time.frameCount;
