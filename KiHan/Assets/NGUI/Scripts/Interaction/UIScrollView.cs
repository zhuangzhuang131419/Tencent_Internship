...
public class UIScrollView : MonoBehaviour
{
    ...
    public delegate void OnDragToEnd();
    public delegate void OnDragging(Vector3 constraint);
    ...
    /// <summary>
	/// Whether dragging will be disabled.
	/// </summary>
    public bool DisableDrag
    {
        set { disableDrag = value; }
        get { return disableDrag; }
    }
    ...
    public float dragToEndOffset = 120f;

    /// <summary>
    /// Event callback to trigger when start drag process . Can be used for additional effects, such as centering on some object.
    /// </summary>
    ...
    /// <summary>
    /// 拖到末尾并超过一定距离后，产生的事件。可用来动态获取更多滚动子项
    /// </summary>

    public OnDragToEnd onDragToEnd;
    ...
    public SpringPanel.OnFinished onRestrictOver;
    ...
    public bool rightCorner()
    {
        ...
        if (canMoveHorizontally)
        {
            if (b.max.x - 10f > clip.x + hx) return false;
        }
    ...
    public bool leftCorner()
    {
        ...
        if (canMoveHorizontally)
        {
            if (b.min.x + 10f < clip.x - hx) return false;
        }

        return true;
    }

    public bool downCorner()
    {
        if (mPanel == null) mPanel = GetComponent<UIPanel>();
        Vector4 clip = mPanel.finalClipRegion;
        Bounds b = bounds;
        ...
        if (canMoveVertically)
        {
            if (b.min.y + 10f < clip.y - hy) return false;
        }

        return true;
    }

    public bool upCorner()
    {
        if (mPanel == null) mPanel = GetComponent<UIPanel>();
        Vector4 clip = mPanel.finalClipRegion;
        Bounds b = bounds;
        ...
        if (canMoveVertically)
        {
            if (b.max.y - 10f > clip.y + hy) return false;
        }
    ...
    public bool RestrictWithinBounds(bool instant, bool horizontal, bool vertical, bool recalculateBounds)
    {
        ...
        return RestrictWithinBounds(instant, horizontal, vertical);
    }
    /// <summary>
    /// Restrict the scroll view's contents to be within the scroll view's bounds.
    /// </summary>
    ...
    public virtual void MoveRelative(Vector3 relative)
    {
        ...
        MessageManager.Instance.serializeToLocal(
                new DragAction(gameObject.GetComponent<UIPanel>(), mPanel.clipOffset, mTrans.localPosition, RemoteModel.Instance.CurrentTime),
        ...
        if (enabled && NGUITools.GetActive(gameObject))
        {
            ...
            else
            {
                ...
                if (onDragToEnd != null)
                {
                    ...
                    {
                        if (constraint.x >= dragToEndOffset)
                        {
                            onDragToEnd();
                        }
                    }
                    ...
                    {
                        if (constraint.y <= -dragToEndOffset)
                        {
                            onDragToEnd();
                        }
                    }
    ...
    public void Drag()
    {
        ...
        if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
        {
            ...
            if (mPlane.Raycast(ray, out dist))
            {
                ...
                Vector3 offset = (currentPos - mLastPos) / damping;
                if (mLastPos.x > currentPos.x && direction == 1)
                {
