...
/// 		- Add Debuger.LogError if no camera with flag MainCamera was found in the scene
/// 
/// 	* EasyTouch class
/// 	-----------------
...
public class EasyTouch : MonoBehaviour {
    ...
    private static bool _instance_enable = true;
    ...
    private Finger[] FingerList = new Finger[10];
	...
	public EasyTouch(){
	    ...
	    {
	        FingerList[i] = new Finger();
	    }
	}
	#endregion


	public bool bTouchDown = true;
	...
	void InitEasyTouch(){
        ...
        { 
			instance = this;
            EasyTouch.SetEnabled(_instance_enable);
        }
    ...
    void OnGUI()
    {
        ...
        {
            rect.x = finger.x - 16;
            rect.y = Screen.height - finger.y - 16;
            rect.width = 32;
            rect.height = 32;

            GUI.DrawTexture(rect, secondFingerTexture);
        }
    }
#endif
    ...
    private Finger[] tmpArray = new Finger[10];
	...
	private void CreateGesture(int touchIndex,EventName message,Finger finger,float actionTime, SwipeType swipe, float swipeLength, Vector2 swipeVector){
		...
		// 3.1.5
		// symonwu  add || message == EventName.On_TouchDown
		if (message == EventName.On_TouchStart || message == EventName.On_TouchUp || message == EventName.On_TouchDown){
		...
		if (!isStartHoverNGUI || message == EventName.On_TouchDown || message == EventName.On_TouchUp){
			...
			gesture.isHoverNGUI = isStartHoverNGUI;
	...
	private bool GetPickeGameObject(ref Finger finger, bool twoFinger=false){
		...
		if (touchCameras.Count>0){
			...
			Debuger.LogWarning("No camera is assigned to EasyTouch");	
		}
    ...
    {
        _instance_enable = enable;

        if (EasyTouch.instance != null && EasyTouch.instance.enable != _instance_enable)
        {
            EasyTouch.instance.enable = _instance_enable;
    ...
    //			EasyTouch.instance.ResetTouches();
		    ...
		    {
			    EasyTouch.instance.bTouchDown = false;
		    }
        }
	}
